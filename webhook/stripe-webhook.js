...
async function handleSuccessfulPayment(session) {
    try {
        ...
        console.log('💰 Montant du paiement:', amount, currency);
        
        // MÊME TRAITEMENT POUR TOUS (paiement ou promo 100%)
        console.log('💾 Sauvegarde dans la base de données...');
        const downloadToken = crypto.randomBytes(32).toString('hex');
        
        // Déterminer le statut (paid ou free selon le montant)
        ...
        `, [
            ...
            console.log('✅ Vente enregistrée ID:', saleId, 'Statut:', status);
            
            // ✅ LE MÊME EMAIL POUR TOUT LE MONDE
            await sendTheBusinessPlanEmail(customerEmail, customerName, downloadToken, saleId);
            
            // Notifier l'admin (avec indication si promo)
            await notifyAdmin(customerEmail, customerName, amount, status === 'free');
            
            console.log(`✅ Terminé: ${customerEmail} - ${amount}${currency} (${status})`);
        });
...
async function sendTheBusinessPlanEmail(email, name, token, saleId) {
    // 🔥 TON DESIGN ORIGINAL GARDÉ À L'IDENTIQUE 🔥
    ...
    try {
        const { data, error } = await resend.emails.send({
            from: 'The Business Plan <onboarding@resend.dev>',
            to: [email],
        ...
        if (error) {
            ...
            console.log(`📧 Email envoyé à ${email}`, data?.id);
        }
    } catch (error) {
...
async function notifyAdmin(customerEmail, customerName, amount, isFree) {
    try {
        const { data, error } = await resend.emails.send({
            from: 'The Business Plan <onboarding@resend.dev>',
            to: [process.env.ADMIN_EMAIL],
            subject: isFree ? '✅ Votre pack THE BUSINESS PLAN est disponible !' : '💰 Nouvelle vente !',
            html: `
                <h2>${isFree ? '🎁 Cadeau' : '💰 Vente'} réalisé !</h2>
                <p><strong>Client:</strong> ${customerName} (${customerEmail})</p>
                <p><strong>Montant:</strong> ${amount}€ ${isFree ? '(PROMO 100%)' : ''}</p>
                <p><strong>Date:</strong> ${new Date().toLocaleString('fr-FR')}</p>
            `
        });
        
        if (error) console.error('Erreur notification admin:', error);
    } catch (error) {
