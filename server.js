...
app.post('/track/visit', async (req, res) => {
    try {
        ...
        db.run(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [
            cleanIp, country, city, deviceType, userAgent.browser || 'Inconnu',
            userAgent.os || 'Inconnu', req.body.page || '/', req.headers.referer || 'Direct', sessionId
        ]);
...
app.get('/download/:token', (req, res) => {
    ...
    db.get(`SELECT s.* FROM sales s WHERE s.download_token = ? AND s.status = 'paid'`, [token], (err, sale) => {
        ...
        res.redirect('https://drive.google.com/uc?export=download&id=TON_ID_FICHIER');
    });
});
            ...
            line_items: [{
                price_data: {
                    ...
                    product_data: {
                        ...
                        description: 'Pack complet de 7 ebooks',
                    },
                    unit_amount: 2700,
                },
                quantity: 1,
...
module.exports = { app, db };// server.js - Serveur Express principal
require('dotenv').config();
const express = require('express');
const sqlite3 = require('sqlite3').verbose();
const path = require('path');
